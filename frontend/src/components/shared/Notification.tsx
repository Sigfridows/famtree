...
import { Bell, Star, Clock, Wrench } from "lucide-react";
...
const DEFAULT_NOTIFICATIONS: NotificationItem[] = [
  {
    ...
    description:
      "Miguel Almánzar ha publicado una nueva reseña sobre la 'Residencia San José'. Haz clic para revisar la calificación y el comentario.",
    time: "1h 54m",
    isUnread: true,
  },
  {
    id: "2",
    type: "appointment",
    title: "Solicitud de Información / Cita",
    description:
      "Un usuario ha agendado una solicitud de visita guiada para el centro 'Hogar del Abuelo' este jueves a las 10:00 AM.",
    time: "3h 12m",
    isUnread: true,
  },
  {
    id: "3",
    type: "status",
    title: "Estado del Centro / Moderación",
    description:
      "Se han actualizado exitosamente la disponibilidad de plazas y los servicios médicos del 'Centro Gerontológico del Este'.",
    time: "1d",
    isUnread: true,
  },
];
...
export default function NotificationsPopover({
  ...
  // Mapas de estilos según la variante visual seleccionada
  const styles = {
    light: {
      ...
      activeTab: "bg-white text-zinc-900 shadow-sm font-semibold",
    ...
    glass: {
      ...
      activeTab:
        "bg-white/15 text-white font-semibold border border-white/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.25)]",
      ...
      iconBg:
        "bg-white/5 border border-white/10 text-zinc-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]",
    },
  }[variant];
  ...
  return (
    <div className="relative inline-block" ref={containerRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-0 right-1 w-2.5 h-2.5 bg-[#82C43C] rounded-full " />
        )}
      </motion.button>
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                className={`text-xs font-semibold transition-colors cursor-pointer ${styles.link}`}
              >
                Ver todo
              </button>
            </div>
              ...
              {notifications.map((item, index) => (
                <div key={item.id}>
                  ...
                  {index < notifications.length - 1 && (
                    <div
                      className={`border-b my-1.5 ${styles.divider}`}
                    />
                  )}
                </div>
              ))}
